...
class BBResponse:
	...
	def __init__(self):
		...
		with open('/home/ubuntu/EENWebsocketRouter/settings/Deployment.csv', mode='r') as infile:
    			reader = csv.reader(infile)
    			self._deviceList = {rows[0]:rows[1] for rows in reader}

	def __addNewData2Window(self, data, speed):
		if len(self.__window)== self.__WINDOWSIZE:
        		self.__window.pop(0)
			self.__speedWindow.pop(0)
    		self.__window.append(data)
		self.__speedWindow.append(speed)
	...
	def __createResponseMSG(self, deviceId, updateRate, command):
		...
		binary_response = ''
			...
			attribute_counter = message.count(",")
			...
			## new bb code
				...
				name, deviceId, speed, updateRate, geoboundary = message.split(",")
				if 'Primary' in geoboundary and float(speed)<=4:
					command=2 # ALARM ON (LIGHT & SOUND)
				elif 'Secondary' in geoboundary and float(speed)<=4:
					command=5 # LIGHTS ON (NO SOUND)
			...
			self.__addNewData2Window(data9dof, float(speed))

			# determine where blackbox is deployed
			deployment = 'Backhoe'
			if deviceId in self._deviceList:
            			deployment = self._deviceList.get(deviceId)
        			if deployment == 'Other':
            				deployment = 'Backhoe' # default option

			# classify activities only after 2 minutes
			deviceStatus = 'IDLE'  # default
			if len(self.__window)== self.__WINDOWSIZE:
				deviceStatus = self.__ml_classifier.classifyActivity(self.__window, deployment, self.__speedWindow)
